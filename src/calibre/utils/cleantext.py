...
def clean_xml_chars(unicode_string):
    ...
    if isinstance(unicode_string, bytes):
        unicode_string = unicode_string.decode(preferred_encoding)
    return ''.join(filter(allowed, unicode_string))


# alias for compatibility
py_clean_xml_chars = clean_xml_chars
native_clean_xml_chars = clean_xml_chars
...
def test_clean_xml_chars():
    ...
    if clean_xml_chars(raw) != 'asda\U00010437xb':
